...
const ChatThread = (props) => {
	...
	useEffect(() => {
		// Fetch User
		Axios.get(`api/users/${id}`).then((res1) => {
			setUser(res1.data.data)
			// Set page
			props.setPage({ name: res1.data.data.name, path: ["chats"] })

			// Fetch Recipient
			Axios.get(`api/users/${to}`).then((res2) => {
				setRecipient(res2.data.data)
				// Set page
				props.setPage({
					name: `${res1.data.data.name} > ${res2.data.data.name}`,
					path: ["chats"],
				})
			})
		})
